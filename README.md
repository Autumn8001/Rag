...
### 3.5 临时沙箱与生命周期安全防护（访客自动清理与鉴权卡控）

系统不仅支持正式租户空间的注册使用，还针对未注册用户以及面试/演示场景提供了高度健壮的安全防护体系：
* **隔离的临时免密访客通道**：访客可一键登录获得动态分配的随机租户空间，数据与其他租户完全物理隔离。
* **两小时生存期自动清理（守护线程）**：后端常驻高可靠的生命周期清理线程 `cleanup_expired_temporary_visitors`，每分钟轮询扫描。一旦访客账号超出 TTL（默认两小时），守护线程将彻底**销毁**该临时租户名下的所有物理文件、PostgreSQL 会话、Wiki 条目、Chroma 中的向量切片以及本地缓存的 BM25 模型 `.pkl` 文件，防止过度占用算力与存储，抵抗而已的灌水攻击。
* **鉴权状态深度防污染卡控**：重构了前端登录识别判定树，引入对正式用户（`is_temporary: false`）的硬性卡控。在正式用户登录成功时强力冲刷并剥离 sessionStorage 中残留的前一轮临时访客有效期（`session_expires_at`），彻底阻断了不同类型账户切换引起的界面倒计时污染问题，确保了多账户状态下的系统安全体验。
